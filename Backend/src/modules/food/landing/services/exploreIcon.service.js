...
/**
 * Upload to the configured local/VPS storage and keep the legacy result shape.
 */
const uploadExploreIconImage = (buffer) =>
    uploadBufferDetailed(buffer, { folder: 'food/explore-icons' });
...
/**
 * Delete the icon record. Disk uploads are retained so an in-flight client cannot
 * lose an image while its cache is still displaying it.
 */
