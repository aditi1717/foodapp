...
const shopSchema = new mongoose.Schema(
  {
    ...
    // These are derived from shopName/ownerPhone at write time.
    shopNameNormalized: {
      type: String,
      trim: true,
    },
     ...
     * Operational toggle controlled by shop dashboard.
     * When false, shop is shown as offline / not accepting orders even within open hours.
     */,
    isAcceptingOrders: {
      type: Boolean,
      default: true,
...
shopSchema.pre("validate", function normalizeDerivedFields(next) {
  const name =
    typeof this.shopName === "string" ? this.shopName : "";
  const normalizedName = name.trim().toLowerCase().replace(/\s+/g, " ");
  this.shopNameNormalized = normalizedName || undefined;
...
shopSchema.index({ ownerPhone: 1 });
shopSchema.index({ shopName: 1 });
shopSchema.index({ shopNameNormalized: 1 });
shopSchema.index({ city: 1 });
shopSchema.index({ "location.city": 1 });
shopSchema.index({ location: "2dsphere" });
shopSchema.index({ shopName: 1, ownerPhone: 1 });
...
shopSchema.index(
  { shopNameNormalized: 1, ownerPhoneLast10: 1 },
  {
    unique: true,
...
export const FoodShop = mongoose.model(
  "FoodShop",
  shopSchema,
);
