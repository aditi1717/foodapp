...
const getAccessibleCategoryFilter = (context) => ({
    $or: [
        { shopId: context.shopId, $or: APPROVED_CATEGORY_FILTER },
        {
            $and: [
...
const resolveCategoryForShop = async (context, body = {}) => {
    ...
    if (!category?._id) {
        throw new ValidationError('Category not found for this shop');
    }
    ...
    if (context.pureVegShop && String(category.foodTypeScope || '') !== 'Veg') {
...
export async function createShopFood(shopId, body = {}) {
    ...
    const doc = await FoodItem.create({
        ...
        subcategoryId: body.subcategoryId && mongoose.Types.ObjectId.isValid(String(body.subcategoryId))
            ? new mongoose.Types.ObjectId(String(body.subcategoryId))
            : undefined,
...
export async function updateShopFood(shopId, foodId, body = {}) {
    const context = await getShopContext(shopId);
    if (!foodId || !mongoose.Types.ObjectId.isValid(String(foodId))) {
        throw new ValidationError('Invalid food id');
    }

    const existing = await FoodItem.findOne({ _id: foodId, shopId }).lean();
    ...
    if (
        ...
        const { categoryObjectId, categoryName } = await resolveCategoryForShop(context, {
    ...
    if (body.subcategoryId !== undefined) {
        if (body.subcategoryId && mongoose.Types.ObjectId.isValid(String(body.subcategoryId))) {
