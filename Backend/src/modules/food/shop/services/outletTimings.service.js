...
import { FoodShopOutletTimings } from '../models/outletTimings.model.js';
...
export async function getOutletTimingsForShop(shopId) {
    if (!shopId || !mongoose.Types.ObjectId.isValid(String(shopId))) {
        throw new ValidationError('Invalid shop id');
    }
    const doc = await FoodShopOutletTimings.findOne({ shopId }).select('timings updatedAt').lean();
    if (!doc) {
        try {
            const shop = await mongoose.model('FoodShop').findById(shopId).select('openingTime closingTime openDays').lean();
            if (shop) {
                const openingTime = shop.openingTime || '09:00';
                const closingTime = shop.closingTime || '22:00';
...
export async function upsertOutletTimingsForShop(shopId, outletTimings) {
    if (!shopId || !mongoose.Types.ObjectId.isValid(String(shopId))) {
        throw new ValidationError('Invalid shop id');
    }
    ...
    const doc = await FoodShopOutletTimings.findOneAndUpdate(
        { shopId },
        { $set: { timings } },
    ...
    // Let's find the primary opening time, closing time, and open days from this payload to save back to FoodShop!
    try {
        ...
        // since FoodShop stored abbreviations from onboarding step 2 ("Mon", "Tue" etc.)
        const openDays = timings.filter(t => t.isOpen).map(t => t.day.slice(0, 3));
        
        await mongoose.model('FoodShop').findByIdAndUpdate(
            shopId,
