...
import { ShopOffer } from '../models/shopOffer.model.js';
import { ShopOfferUsage } from '../models/shopOfferUsage.model.js';
...
export async function createShopProductOffer(shopId, body = {}) {
    const rid = ensureObjectId(shopId, 'shop id');
...
export async function listShopProductOffers(shopId) {
    const rid = ensureObjectId(shopId, 'shop id');
    const list = await ShopOffer.find({ shopId: rid }).sort({ createdAt: -1 }).lean();
    return list.map((o) => ({
        ...o,
...
export async function listPublicShopProductOffers(shopId, authUser = null) {
    const rid = ensureObjectId(shopId, 'shop id');
    const now = new Date();
    const list = await ShopOffer.find({ shopId: rid })
...
export async function deleteShopProductOffer(shopId, offerId) {
    const rid = ensureObjectId(shopId, 'shop id');
    const oid = ensureObjectId(offerId, 'offer id');
    const offer = await ShopOffer.findOne({ _id: oid, shopId: rid }).lean();
    if (!offer) return null;
    await ShopOffer.deleteOne({ _id: oid });
    return { id: String(offerId) };
}

export async function updateShopProductOffer(shopId, offerId, body = {}) {
    const rid = ensureObjectId(shopId, 'shop id');
    const oid = ensureObjectId(offerId, 'offer id');
    const existing = await ShopOffer.findOne({ _id: oid, shopId: rid }).lean();
    ...
    const updated = await ShopOffer.findOneAndUpdate(
        { _id: oid, shopId: rid },
        {
            $set: {
                title,
