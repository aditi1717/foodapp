...
export const getWalletDetailsController = async (req, res, next) => {
    try {
        ...
        const walletData = await getWalletWithTransactions('shop', shopId, { page, limit });

        // Fetch withdrawals to merge pending and rejected ones into transactions history
        const withdrawals = await FoodShopWithdrawal.find({ shopId }).sort({ createdAt: -1 }).lean();
...
export const createDepositOrderController = async (req, res, next) => {
    try {
        ...
        const receipt = `resto_wt_${String(shopId).slice(-8)}_${Date.now()}`;
...
export const withdrawFromWalletController = async (req, res, next) => {
    try {
        ...
        // Resolve actual FoodShop ID
        const shopProfile = await FoodShop.findOne({ _id: ownerId }).select('_id accountNumber ifscCode bankName accountHolderName').lean();
        const shopId = shopProfile?._id;
        if (!shopId) return sendError(res, 404, 'Shop profile not found');

        // Check if there is enough balance in the wallet
        const walletData = await getWalletWithTransactions('shop', shopId, { page: 1, limit: 1 });
        ...
        const result = await lockWalletAmount('shop', shopId, withdrawAmount);
        ...
        const withdrawal = new FoodShopWithdrawal({
            ...
            bankDetails: {
                ...
                accountHolderName: shopProfile.accountHolderName || shopProfile.shopName || ''
            }
        });
