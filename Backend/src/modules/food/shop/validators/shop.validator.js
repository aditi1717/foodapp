...
const requiredBooleanSchema = z.preprocess((value) => {
    ...
    return value;
}, z.boolean({ required_error: 'Please select whether the shop is pure veg' }));
...
const shopRegisterSchema = z.object({
    ...
    pureVegShop: requiredBooleanSchema,
...
export const validateShopRegisterDto = (body) => {
    const result = shopRegisterSchema.safeParse(body);
    if (!result.success) {
