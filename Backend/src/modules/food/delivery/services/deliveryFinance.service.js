...
export const getDeliveryPartnerWalletEnhanced = async (deliveryPartnerId) => {
    ...
    const [cashLimitSettings, earningsAgg, cashCollectedAgg, bonusAgg, withdrawalAgg, payoutAgg, withdrawalsList, depositList, completedDepositsAgg] = await Promise.all([
    ...
    const cashSettledAgg = codOrderIds.length
        ...
        : [];
    const cashDepositsSum = Number(completedDepositsAgg?.[0]?.total) || 0;
    ...
    // Pocket balance is calculated strictly from wallet deposits and withdrawal requests.
    const totalWithdrawn = legacyApprovedWithdrawn;
    ...
    // Pocket Balance = Deposits - Withdrawals - Pending Withdrawals
    const pocketBalance = Math.max(0, cashDepositsSum - totalWithdrawn - pendingWithdrawals);
    ...
    const transactions = [
        ...
        ...(withdrawalsList || []).map(w => ({
            ...
            type: 'withdrawal',
            amount: w.amount,
            status: w.status === 'pending' ? 'Pending' : (['approved', 'processed'].includes(w.status) ? 'Completed' : 'Rejected'),
            date: w.createdAt,
