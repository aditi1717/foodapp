...
const upsertSchema = z.object({
    name: z.string().min(1, 'Subcategory name is required').max(200).optional(),
...
export const validateSubcategoryListQuery = (query) => {
...
export const validateSubcategoryUpsertDto = (body) => {
    const result = upsertSchema.safeParse(body);
    if (!result.success) {
        throw new ValidationError(result.error.errors[0]?.message || 'Invalid subcategory data');
    }
    const d = result.data;
...
export const validateSubcategoryRejectDto = (body) => {
    const result = rejectSchema.safeParse(body);
    if (!result.success) {
        throw new ValidationError(result.error.errors[0]?.message || 'Invalid subcategory rejection data');
    }
    return result.data;
};
