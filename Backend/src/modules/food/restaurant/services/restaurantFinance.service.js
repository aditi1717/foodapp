...
export async function getRestaurantFinance(restaurantId, query = {}) {
    ...
    const [restaurantWalletBalance, takeawayCashInHandAgg] = await Promise.all([
    ...
    const restaurantWalletAvailableBalance = Math.max(
        0,
        Number(restaurantWalletBalance?.availableBalance ?? restaurantWalletBalance?.balance ?? 0) || 0
    );
    const takeawayCashLimitLeft = Math.max(0, restaurantWalletAvailableBalance - takeawayCashInHand);
    ...
    const currentCycle = {
        ...
        payoutDate: null,
        takeawayCod: {
            walletBalance: restaurantWalletAvailableBalance,
