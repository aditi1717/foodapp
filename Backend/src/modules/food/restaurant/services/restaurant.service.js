...
import { getPriorityListingRestaurantIds } from '../../shared/subscription.service.js';
...
const attachPriorityListingFlag = (restaurants = [], priorityRestaurantIds = []) => {
    const priorityIdSet = new Set((priorityRestaurantIds || []).map((id) => String(id)));
    return (restaurants || []).map((restaurant) => ({
        ...restaurant,
        isPriorityListing: priorityIdSet.has(String(restaurant?._id || restaurant?.id || restaurant?.restaurantId || '')),
    }));
};
...
export const listApprovedRestaurants = async (query = {}) => {
    ...
    const priorityRestaurantIds = shouldApplyPriorityListing
        ? await getPriorityListingRestaurantIds(visibleRestaurantObjectIds.map((id) => String(id)))
        : [];
    const priorityRestaurantObjectIds = priorityRestaurantIds
        .filter((id) => mongoose.Types.ObjectId.isValid(id))
        .map((id) => new mongoose.Types.ObjectId(id));
    ...
    if (lat !== null && lng !== null && wantsGeo) {
        ...
        if (!priorityRestaurantObjectIds.length) {
        ...
        const priorityFilter = { ...filter, _id: { $in: priorityRestaurantObjectIds } };
        const normalFilter = { ...filter, _id: { $nin: priorityRestaurantObjectIds, $in: visibleRestaurantObjectIds } };
        ...
        const [priorityCountDocs, normalCountDocs] = await Promise.all([
            FoodRestaurant.aggregate([...priorityPipeline, { $count: 'count' }]),
            FoodRestaurant.aggregate([...normalPipeline, { $count: 'count' }])
        ]);
    ...
    if (!priorityRestaurantObjectIds.length) {
        ...
        const [priorityCount, normalCount] = await Promise.all([
            FoodRestaurant.countDocuments(priorityFilter),
            FoodRestaurant.countDocuments(normalFilter)
        ]);
        ...
        const [priorityDocs, normalDocs] = await Promise.all([
            priorityLimit > 0
                ? FoodRestaurant.find(priorityFilter)
                    .select(Object.keys(projection).join(' '))
                    .sort(sort)
                    .skip(prioritySkip)
                    .limit(priorityLimit)
                ...
                : Promise.resolve([]),
            normalLimit > 0
                ? FoodRestaurant.find(normalFilter)
                    .select(Object.keys(projection).join(' '))
                    .sort(sort)
                    .skip(normalSkip)
                    .limit(normalLimit)
        ...
        restaurantsRaw = [...priorityDocs, ...normalDocs];
    }
    ...
    const restaurants = (restaurantsRaw || []).map((r) => ({
        ...
        isPriorityListing: priorityRestaurantIds.includes(String(r._id)),
