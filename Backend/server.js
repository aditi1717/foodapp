...
let scheduledOrderInterval = null;
...
const gracefulShutdown = async (signal) => {
    ...
    server.close(async () => {
        try {
            ...
            if (scheduledOrderInterval) clearInterval(scheduledOrderInterval);
    ...
    try {
        ...
        const runScheduledOrdersCheck = async () => {
            try {
                ...
                logger.error(`Scheduled orders activation error: ${err.message}`);
            }
        };
        runScheduledOrdersCheck();
        scheduledOrderInterval = setInterval(runScheduledOrdersCheck, 60 * 1000); // Check every minute
