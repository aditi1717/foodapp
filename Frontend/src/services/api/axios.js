...
function getModuleFromUrl(url = "") {
  ...
  // Shop/shop detection - frontend uses "shop" module name; backend routes stay /food/shop/
  if (
    normalized.includes("/food/shop/") || 
    normalized.includes("/auth/shop") || 
    normalized.includes("/shop/")
  ) {
    // Exception: /food/shops (plural) is usually a public user app route
    if (normalized.includes("/food/shops") && !normalized.includes("/food/shop/")) {
       return "user";
    }
    return "shop";
  }
...
function tokenMatchesModule(token, module) {
  ...
  // "shop" is the frontend module name; backend always issues SHOP role tokens
  if (module === "shop" || module === "shop") return role === "SHOP";
...
function getAccessToken(config) {
  ...
  try {
    ...
    if (moduleToken && tokenMatchesModule(moduleToken, module)) return moduleToken;
    
    // 2. USER legacy fallback used by older user-auth flows
    if (module === "user") {
      const legacyUserToken = localStorage.getItem("auth_customer");
      if (legacyUserToken && tokenMatchesModule(legacyUserToken, module)) {
        return legacyUserToken;
      }
    }

    // 3. Fallback to generic token only when its role matches the requested module
    if (module !== "admin") {
      const genericToken = localStorage.getItem("accessToken");
      if (genericToken && tokenMatchesModule(genericToken, module)) {
        return genericToken;
      }
    }
    return null;
