        ...
        navigator.geolocation.getCurrentPosition(
          async (pos) => {
            try {
              ...
              // Reverse geocode (Google Maps / Nominatim / BigDataCloud)
              let addr
              ...
              // Check if location has placeholder values - don't save placeholders if there is already a stored location
              const hasPlaceholder =
              ...
              if (hasPlaceholder && storedLocation && !forceFresh) {
                debugWarn("?? Skipping save - location contains placeholder values and we already have a stored location:", finalLoc)
                // Don't overwrite existing valid location with placeholders during background checks
                const coordOnlyLoc = {
                  latitude,
                  longitude,
                  accuracy: accuracy || null,
      ...
      watchIdRef.current = navigator.geolocation.watchPosition(
        async (pos) => {
          try {
            ...
            let addr
            if (lastResolvedAddressRef.current && lastResolvedAddressRef.current.formattedAddress) {
            ...
            const storedLocation = localStorage.getItem("userLocation")
            if (hasPlaceholder && storedLocation) {
              debugWarn("?? Skipping live location update - contains placeholder values and we already have a stored location:", loc)
        ...
        if (navigator.permissions && navigator.permissions.query) {
          try {
            ...
            if (result.state === 'granted') {
              ...
              // ✅ KEY FIX: Subscribe to permission change so we react immediately when
              // the user clicks "Allow" in the browser dialog — no page refresh needed.
              result.onchange = () => {
                if (result.state === 'granted') {
                  debugLog('?? Permission just granted by user — fetching location now');
                  // Clear any stale cached location so we get a real GPS fix
                  try { localStorage.removeItem('userLocation'); } catch { /* ignore */ }
                  getLocation(true, true, true)
                    .then((loc) => {
                      if (loc && loc.formattedAddress !== 'Select location' && loc.city !== 'Current Location') {
                        setLocation(loc);
                        setPermissionGranted(true);
                      }
                    })
                    .catch((err) => {
                      debugWarn('?? Post-permission-grant location fetch failed:', err.message);
                    });
                }
              };
            }
