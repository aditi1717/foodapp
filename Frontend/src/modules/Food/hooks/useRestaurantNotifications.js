...
const buildScheduledPreparationNotification = (orderData = {}) => {
  const orderId = orderData.orderId || orderData.orderMongoId || 'Order';
  const scheduledAt = orderData.scheduledAt
  ...
  return {
    title: `Start preparing #${orderId}`,
    body: `Scheduled order should be prepared now for ${scheduledLabel}.`,
...
export const useRestaurantNotifications = () => {
  ...
  const shouldProcessOrderAlert = (orderData = {}) => {
    ...
    const isPreparingScheduled = status === "preparing" && (orderData?.isScheduled || orderData?.scheduledAt);
    const isReminder = orderData?.isPreparationReminder === true;
    if (status && status !== "created" && status !== "confirmed" && !isPreparingScheduled && !isReminder) return false;
  ...
  const showBackgroundBrowserNotification = async (orderData, buildNotification) => {
    ...
    const notificationOptions = buildNotification(orderData);
  ...
  const handleIncomingOrderAlert = useCallback((orderData) => {
    ...
    if (orderData?.isPreparationReminder) {
      showBackgroundBrowserNotification(orderData, buildScheduledPreparationNotification);
    } else {
  ...
  useEffect(() => {
    ...
    socketRef.current.on('play_notification_sound', (data) => {
      ...
      startAlertLoop();
      if (normalizedData.isPreparationReminder) {
        showBackgroundBrowserNotification(normalizedData, buildScheduledPreparationNotification);
      } else {
    ...
    socketRef.current.on('scheduled_preparation_reminder', (data) => {
      debugLog('?? Scheduled preparation reminder:', data);
      const normalizedData = {
        orderId: data?.orderId || data?.order_id,
        orderMongoId: data?.orderMongoId || data?.order_mongo_id,
        ...data,
      ...
      toast.info(
        normalizedData?.message ||
          `It is time to start preparing order ${normalizedData?.orderId || ''}`.trim(),
        { duration: 8000 },
      );

      showBackgroundBrowserNotification(
        normalizedData,
        buildScheduledPreparationNotification,
      );
    ...
    socketRef.current.on('order_status_update', (data) => {
      ...
      const dispatchStatus = String(data?.dispatchStatus || data?.dispatch_status || '').toLowerCase();
      ...
      if (dispatchStatus === 'accepted') {
        ...
        toast.success(
          readableOrderId
            ? `Delivery partner accepted order ${readableOrderId}`
            : 'Delivery partner accepted the order',
          { duration: 5000 },
        );
      } else if (dispatchStatus === 'unassigned' && (reasonType === 'passed' || reasonType === 'timed_out')) {
        const readableOrderId = String(data?.orderId || data?.order_id || '').trim();
        const passText = reasonType === 'timed_out' ? 'timed out' : 'passed';
        toast.warning(
          readableOrderId
            ? `Delivery partner ${passText} for order ${readableOrderId}. Please reassign.`
            : `Delivery partner ${passText}. Please reassign.`,
          { duration: 6000 },
        );
      }
