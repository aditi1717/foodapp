...
const USER_SESSION_PREFERENCE_KEYS = ["userVegMode", "userVegModePreference", "food-under-250-filters"]
...
export function ProfileProvider({ children }) {
  ...
  const [vegModePreference, setVegModePreference] = useState(() => {
    const saved = localStorage.getItem("userVegModePreference")
  ...
  useEffect(() => {
    if (isAuthenticated) {
      localStorage.setItem("userVegModePreference", vegModePreference === "pure-veg" ? "pure-veg" : "all")
    }
  }, [vegModePreference, isAuthenticated])
  ...
  const addFavorite = useCallback((shop) => {
    setFavorites((prev) => {
      if (!prev.find(fav => fav.slug === shop.slug)) {
        return [...prev, shop]
      }
      return prev
    })
  ...
  const addDishFavorite = useCallback((dish) => {
    setDishFavorites((prev) => {
      if (!prev.find(fav => fav.id === dish.id && fav.shopId === dish.shopId)) {
        return [...prev, dish]
      }
      return prev
    })
  ...
  const removeDishFavorite = useCallback((dishId, shopId) => {
    setDishFavorites((prev) => 
      prev.filter(fav => !(fav.id === dishId && fav.shopId === shopId))
    )
  }, [])

  const isDishFavorite = useCallback((dishId, shopId) => {
    return dishFavorites.some(fav => fav.id === dishId && fav.shopId === shopId)
  }, [dishFavorites])
...
export function useProfile() {
  ...
  if (!context) {
    ...
    return {
      ...
      vegModePreference: "all",
      setVegModePreference: () => debugWarn("ProfileProvider not available")
    }
  }
  return context
}
