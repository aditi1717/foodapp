...
export default function ShopWithdrawal() {
  ...
  const handleReject = async (id) => {
    try {
      ...
      if (response?.data?.success) {
        toast.success("Withdrawal request rejected successfully. Funds have been returned to available balance.")
          ...
          <DialogContent className="max-w-md bg-white p-0">
              ...
              <div className="px-6 pb-6 space-y-4">
                ...
                {selectedRequest.rejectionReason && (
                  <div>
                    ...
                    <p className="text-sm font-medium text-slate-900 mt-1 text-red-600 font-medium">{selectedRequest.rejectionReason}</p>
                  </div>
                )}
              </div>
            )}
            <DialogFooter className="px-6 pb-6">
              <button
                ...
                className="px-4 py-2 text-sm font-medium rounded-lg bg-indigo-600 text-white hover:bg-indigo-700 transition-all"
              >
                Close
              </button>
            </DialogFooter>
