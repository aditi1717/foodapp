...
const normalizeWalletRow = (row = {}) => {
  ...
  return {
    ...row,
    name: row.name || row.deliveryPartnerName || row.deliveryBoyName || row.userName || "—",
    deliveryIdString: row.deliveryIdString || row.deliveryId || row.deliveryPartnerId || row.partnerId || "—",
      ...
      if (res?.data?.success) {
        ...
        try {
          ...
          if (partnersRes?.data?.success) {
            ...
            partners.forEach((partner) => {
              const partnerData = {
                  name: partner.name || partner.deliveryPartnerName || partner.fullName || "—",
                  phone: partner.phone || "",
                  cashInHand: toNumber(
                    partner.cashInHand,
            ...
            if (earningsRows.length > 0) {
              ...
              earningsRows.forEach((item) => {
                ...
                const prev = earningsMap.get(partnerKey) || {
                  ...
                  deliveryIdString: item.deliveryIdString || item.deliveryId || item.deliveryPartnerId || "—",
                }
        ...
        if (partnerMap.size > 0) {
          nextRows = nextRows.map((row) => {
            ...
            return {
              ...row,
              name: row.name === "—" ? partnerMatch.name : row.name,
              cashInHand,
            }
          })
        }
