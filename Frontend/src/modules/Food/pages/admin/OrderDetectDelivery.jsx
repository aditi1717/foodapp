...
const transformOrder = (order, index) => {
  ...
  const offeredEntries = Array.isArray(order?.dispatch?.offeredTo) ? order.dispatch.offeredTo : []
  const latestOffer = offeredEntries.length ? offeredEntries[offeredEntries.length - 1] : null
  const latestOfferedPartner =
    latestOffer?.partnerId && typeof latestOffer.partnerId === "object"
      ? latestOffer.partnerId
      : null
  const requestedDeliveryBoyName =
    latestOfferedPartner?.name ||
    order?.requestedDeliveryBoyName ||
    null
  const requestedDeliveryBoyNumber =
    latestOfferedPartner?.phone ||
    order?.requestedDeliveryBoyNumber ||
    null
  ...
  return {
    ...
    requestedDeliveryBoyNumber,
    latestOfferAction: String(latestOffer?.action || "").toLowerCase(),
...
export default function OrderDetectDelivery() {
  ...
  const handleAdminStatusChange = async (order, nextStatus) => {
    ...
    if (normalizedNextStatus === "approve_user_unavailable") {
      const loadingKey = `status:${orderId}:approve_user_unavailable`
      setActionLoadingKey(loadingKey)
      try {
        const response = await adminAPI.updateOrderStatus(orderId, {
          orderStatus: "cancelled_by_user_unavailable",
          reason: "User unavailable request approved by admin",
        })
    ...
    if (normalizedNextStatus === "reject_user_unavailable") {
      const loadingKey = `status:${orderId}:reject_user_unavailable`
      setActionLoadingKey(loadingKey)
      try {
        const response = await adminAPI.updateOrderStatus(orderId, {
          orderStatus: "cancelled_by_admin",
          reason: "User unavailable request rejected by admin",
        })
        if (!response?.data?.success) {
          throw new Error(response?.data?.message || "Failed to reject user unavailable request")
        }
        const paymentMethod = String(order?.originalOrder?.payment?.method || "").toLowerCase()
        if (["razorpay", "wallet"].includes(paymentMethod)) {
          toast.success(`Rejected. Refund will be processed to user for order #${order.orderId}.`)
        } else {
          toast.success(`Rejected. Cash order #${order.orderId}, no refund needed.`)
        }
        await fetchOrders({ silent: true })
      } catch (statusError) {
        toast.error(statusError?.response?.data?.message || statusError?.message || "Failed to reject request")
      } finally {
        setActionLoadingKey("")
      }
      return
    ...
    if (!["picked_up", "delivered"].includes(normalizedNextStatus)) return

    const loadingKey = `status:${orderId}:${normalizedNextStatus}`
    setActionLoadingKey(loadingKey)
    try {
      const response = await adminAPI.updateOrderStatus(orderId, { orderStatus: normalizedNextStatus })
      if (!response?.data?.success) {
