...
export default function FoodsList() {
  ...
  const openEditFoodModal = (food) => {
    ...
    setFoodForm({
      ...
      subcategoryName: String(food.subcategoryName || ""),
  ...
  useEffect(() => {
    ...
    const loadSubcategoryOptions = async () => {
      try {
        const params = { limit: 1000, categoryId: foodForm.categoryId }
        const res = await adminAPI.getSubcategories(params)
          ...
          ? list
              .map((sc) => {
                return {
                  ...
                  foodTypeScope: String(sc.foodTypeScope || "Both"),
                }
              })
              .filter((sc) => sc.name)
          : []
  ...
  }, [showFoodFormModal, foodForm.categoryId])
  ...
  const handleFoodFormSubmit = async () => {
    ...
    try {
      ...
      const payload = {
        // Auto-derive from selected scope. If scope is Both/missing, keep Non-Veg fallback.
        foodType: (() => {
          const selectedSubcategory = subcategoryOptions.find((item) => String(item.id) === String(foodForm.subcategoryId))
          const selectedCategory = categoryOptions.find((item) => String(item.id) === String(foodForm.categoryId))
          const subScope = String(selectedSubcategory?.foodTypeScope || "").trim()
          const categoryScope = String(selectedCategory?.foodTypeScope || "").trim()
                      ...
                      {categoryOptions
                        ...
                        .map((c) => (
                          <button
                            ...
                            type="button"
                            onClick={() => {
                              setFoodForm((prev) => ({ ...prev, categoryId: c.id, categoryName: c.name, subcategoryId: "", subcategoryName: "" }))
                              setCategoryPopoverOpen(false)
                            }}
              ...
              <div>
                ...
                <Popover open={subcategoryPopoverOpen} onOpenChange={setSubcategoryPopoverOpen}>
                  <PopoverTrigger asChild>
                    <button
                      type="button"
                      disabled={!foodForm.categoryId}
                      className="w-full px-3 py-2.5 border border-slate-300 rounded-lg text-sm bg-white text-left flex items-center justify-between disabled:bg-slate-100 disabled:text-slate-400"
                    >
                      <span className={foodForm.subcategoryName ? "text-slate-900" : "text-slate-400"}>
                        {foodForm.subcategoryName || (foodForm.categoryId ? "Select subcategory" : "Select category first")}
                      </span>
                      <ChevronDown className="w-4 h-4 text-slate-500" />
                    </button>
                  </PopoverTrigger>
                  <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-2" align="start">
                    ...
                    <div className="max-h-56 overflow-y-auto">
                      {subcategoryOptions
                        .filter((sc) => {
                          const q = String(subcategorySearch || "").trim().toLowerCase()
                          if (!q) return true
                          return String(sc.name || "").toLowerCase().includes(q)
                        })
                        .map((sc) => (
                          <button
                            key={sc.id}
                            type="button"
                            onClick={() => {
                              setFoodForm((prev) => ({ ...prev, subcategoryId: sc.id, subcategoryName: sc.name }))
                              setSubcategoryPopoverOpen(false)
                            }}
                            className={`w-full text-left px-3 py-2 rounded-md text-sm hover:bg-slate-100 ${
                              String(foodForm.subcategoryName || "") === String(sc.name) ? "bg-slate-100 font-medium" : ""
                            }`}
                          >
                            <span>{sc.name}</span>
                          </button>
                      ...
                      {subcategoryOptions.length === 0 && (
                        <div className="px-3 py-2 text-sm text-slate-500">No subcategories found</div>
                      )}
                    </div>
                  </PopoverContent>
                </Popover>
