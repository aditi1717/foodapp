...
export default function CampaignOrderReport() {
  ...
  const filteredOrders = useMemo(() => {
    ...
    if (filters.shop !== "All shops") {
      result = result.filter(o => o.shop === filters.shop)
    }
  ...
  const activeFiltersCount = (filters.campaign !== "All Campaignes" ? 1 : 0) + (filters.shop !== "All shops" ? 1 : 0) + (filters.customer !== "All customers" ? 1 : 0) + (filters.time !== "All Time" ? 1 : 0)
          ...
          <div className="flex flex-col lg:flex-row lg:items-end gap-3 w-full">
            ...
            <div className="flex items-end gap-2 pt-1">
              ...
              <button 
                ...
                className={`px-4 py-2 text-xs font-medium rounded-lg bg-brand-500 text-white hover:bg-brand-600 transition-all flex items-center gap-2 relative ${
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4 w-full">
            ...
            <div className="flex items-center gap-3">
              <div className="relative flex-1 sm:flex-initial min-w-[220px]">
                <input
                  type="text"
                  placeholder="Search by Order ID, Shop, Customer"
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  className="pl-3 pr-9 py-2 w-full text-xs rounded-lg border border-slate-300 bg-white focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-brand-500"
                />
              ...
              <tbody className="bg-white divide-y divide-slate-100">
                {filteredOrders.map((order) => (
                  <tr key={order.sl} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-3 py-2 whitespace-nowrap">
                      <span className="text-xs text-slate-700">{order.shop}</span>
                    </td>
                    <td className="px-3 py-2 whitespace-nowrap">
                    ...
                    <td className="px-3 py-2 whitespace-nowrap text-center">
                      <div className="flex items-center justify-center gap-1.5">
                        <button className="p-1 text-brand-600 hover:text-brand-800 hover:bg-brand-50 rounded transition-colors">
