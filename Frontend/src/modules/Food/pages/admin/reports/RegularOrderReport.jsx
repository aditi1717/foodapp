...
const resolveReportOrderType = (order = {}) => {
  const scheduleLabel = order?.isScheduled ? "Scheduled" : "Instant"
  ...
  const fulfillmentLabel =
    normalizeLower(fulfillmentRaw) === "takeaway" ? "Takeaway" : "Delivery"

  return `${scheduleLabel} + ${fulfillmentLabel}`
}

const isTakeawayOrderType = (value) =>
  normalizeLower(value).includes("takeaway")
...
export default function RegularOrderReport() {
      ...
      try {
        ...
        if (response.data?.success) {
          ...
          const transformedOrders = rawOrders.map((order) => {
            ...
            const orderType = resolveReportOrderType(order)
            const deliveryBoyEarningDisplay = isTakeawayOrderType(orderType)
              ? "N/A"
            ...
            <tr>
              ...
              <td class="num">${htmlEscape(
                typeof order.deliveryBoyEarningDisplay === "string"
                  ? order.deliveryBoyEarningDisplay
                  : toAmountNumber(order.deliveryBoyEarningDisplay).toFixed(2),
              )}</td>
  ...
  const formatDeliveryBoyEarning = (value) =>
                ...
                {paginatedOrders.length === 0 ? (
                  ...
                  paginatedOrders.map((order, index) => (
                    <tr key={order.orderId} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-1.5 py-1">
                        <span className="text-[10px] text-amber-600 font-semibold">{formatDeliveryBoyEarning(order.deliveryBoyEarningDisplay)}</span>
                      </td>
                      <td className="px-1.5 py-1">
