...
const resolveOrderScheduledAt = (order = {}) =>
  order?.scheduledAt ||
...
const isScheduledOrder = (order = {}) =>
  Boolean(order?.isScheduled || resolveOrderScheduledAt(order))
...
export default function OrdersPage({ statusKey = "all" }) {
  ...
  const fetchOrders = useCallback(async (options = {}) => {
    ...
    try {
      ...
      const filteredOrders =
        statusKey === "scheduled"
          ? nextOrders.filter((order) => isScheduledOrder(order))
          : nextOrders
  ...
  const normalizedOrders = useMemo(() => {
    ...
    return safeOrders.filter(Boolean).map((order) => {
      ...
      const scheduledAt = resolveOrderScheduledAt(order)
      ...
      const isTakeaway = fulfillmentType === "takeaway" || fulfillmentType === "pickup"
      const scheduleLabel = isScheduled ? "Scheduled" : "Instant"
      const modeLabel = isTakeaway ? "Takeaway" : "Delivery"
      const orderTypeLabel = `${scheduleLabel} + ${modeLabel}`
      ...
      return {
        ...
        isScheduled,
        scheduledAt,
        fulfillmentType: isTakeaway ? "takeaway" : "delivery",
        orderTypeLabel,
        deliveryPartnerName,
        deliveryPartnerPhone,
        deliveryType: isTakeaway ? "Takeaway" : (order.deliveryType || "Home Delivery"),
