...
const resolveScheduleMode = (order = {}) =>
  order?.isScheduled || order?.scheduledAt ? "Scheduled" : "Instant"

const resolveFulfillmentLabel = (order = {}) =>
  String(order?.fulfillmentType || "").toLowerCase() === "takeaway"
    ? "Takeaway"
    : "Delivery"

const resolveOrderTypeLabel = (order = {}) =>
  `${resolveScheduleMode(order)} + ${resolveFulfillmentLabel(order)}`

const resolveCashInHandAmount = (order = {}) => {
  if (String(order?.fulfillmentType || "").toLowerCase() !== "takeaway") return null
  const amount = Number(order?.cashInHandAmount || 0)
  return Number.isFinite(amount) ? amount : 0
}

const resolveCashInHandStatus = (order = {}) => {
  if (String(order?.fulfillmentType || "").toLowerCase() !== "takeaway") return "N/A"

  const status = String(order?.cashInHandStatus || "").toLowerCase()
  if (status === "paid_to_admin") return "Paid to admin"
  if (status === "unpaid_to_admin") return "Unpaid to admin"

  const paymentMethod = String(order?.paymentMethod || "").toLowerCase()
  if (paymentMethod && paymentMethod !== "cash") return "Paid online"
  return "N/A"
}
...
export default function HubFinance() {
  ...
  const takeawayCashSummary = useMemo(() => {
    const summary = financeData?.currentCycle?.takeawayCod || {}
    return {
      walletBalance: Number(summary.walletBalance || 0),
      cashInHand: Number(summary.cashInHand || 0),
      remainingCashLimit: Number(summary.remainingCashLimit || 0),
      activeCashOrders: Number(summary.activeCashOrders || 0),
    }
  }, [financeData])

  const renderTakeawayCashCards = () => (
    <div>
      <h2 className="text-base font-bold text-gray-900 mb-3">Takeaway cash limit</h2>
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-white rounded-lg p-4">
          <p className="text-xs font-semibold uppercase tracking-wide text-gray-500 mb-2">Cash in hand</p>
          <p className="text-xl font-bold text-gray-900">{formatMoney(takeawayCashSummary.cashInHand)}</p>
          <p className="text-[11px] text-gray-600 mt-2">
            Unsettled takeaway COD amount
          </p>
        </div>
        <div className="bg-white rounded-lg p-4 border border-emerald-100">
          <p className="text-xs font-semibold uppercase tracking-wide text-emerald-700 mb-2">Cash limit left</p>
          <p className="text-xl font-bold text-emerald-700">{formatMoney(takeawayCashSummary.remainingCashLimit)}</p>
          <p className="text-[11px] text-gray-600 mt-2">
            Amount left to accept new takeaway cash orders
          </p>
        </div>
        <div className="bg-white rounded-lg p-4">
        ...
        <div className="bg-white rounded-lg p-4">
          <p className="text-xs font-semibold uppercase tracking-wide text-gray-500 mb-2">COD orders</p>
          <p className="text-xl font-bold text-gray-900">{takeawayCashSummary.activeCashOrders}</p>
          <p className="text-[11px] text-gray-600 mt-2">
            Active takeaway cash orders
          </p>
        </div>
      </div>
    </div>
            ...
            <table class="orders-table">
              <thead>
                <tr>
                  ...
                  <th style="width: 28%;">Order Type / Items</th>
                  <th style="width: 8%;">Qty</th>
                  <th style="width: 11%;">Amount / Cash</th>
                  <th style="width: 12%;">Earning / Admin</th>
                </tr>
              </thead>
              <tbody>
    ...
    try {
      ...
      const bodyRowsHtml = orders
        .map((order) => {
          ...
          const fulfillmentType = resolveFulfillmentLabel(order)
          const cashInHandAmount = resolveCashInHandAmount(order)
          const cashInHandStatus = resolveCashInHandStatus(order)
          ...
          return `
            <tr>
              ...
              <td>${htmlEscape(orderType)}</td>
              <td>${htmlEscape(scheduleMode)}</td>
              <td>${htmlEscape(fulfillmentType)}</td>
              <td class="num">${htmlEscape(cashInHandAmount === null ? "N/A" : cashInHandAmount.toFixed(2))}</td>
              <td>${htmlEscape(cashInHandStatus)}</td>
      ...
      <div className="flex-1 overflow-y-auto px-4 pt-6 pb-28">
        {activeTab === "payouts" && (
          <div className="space-y-6">
            ...
            {!loading && renderTakeawayCashCards()}
                          ...
                          <table className="w-full min-w-[1240px]">
                            <thead className="bg-gray-50 border-b border-gray-200">
                              <tr>
                                ...
                                <th className="px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wide">Order Type</th>
                                <th className="px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wide">Schedule</th>
                                <th className="px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wide">Mode</th>
                                <th className="px-4 py-3 text-right text-xs font-semibold text-gray-700 uppercase tracking-wide">Cash In Hand</th>
                                <th className="px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wide">Admin Cash Status</th>
                              ...
                              {paginatedPayoutOrders.map((order, index) => {
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    <td className="px-4 py-3 text-sm text-gray-700">{orderType}</td>
                                    <td className="px-4 py-3 text-sm text-gray-700">{scheduleMode}</td>
                                    <td className="px-4 py-3 text-sm text-gray-700">{fulfillmentLabel}</td>
                                    <td className="px-4 py-3 text-sm text-right text-gray-700">{cashInHandAmount === null ? "N/A" : formatMoney(cashInHandAmount)}</td>
                                    <td className="px-4 py-3 text-sm text-gray-700">{cashInHandStatus}</td>
