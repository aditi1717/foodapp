...
const formatScheduledDateTime = (dateInput) => {
  if (!dateInput) return "";
  const d = new Date(dateInput);
...
const resolveScheduledAt = (orderLike = {}) =>
  orderLike?.scheduledAt ||
  orderLike?.scheduleAt ||
  orderLike?.scheduled_at ||
  orderLike?.deliveryScheduledAt ||
  null;

const isPendingAcceptanceStatus = (statusLike) => {
  const normalizedStatus = String(statusLike || "").toLowerCase();
  return ["created", "confirmed"].includes(normalizedStatus);
};

const isScheduledOrderReadyForAcceptance = (
  orderLike = {},
  now = Date.now(),
  leadMinutes = 30,
) => {
  const scheduledAt = resolveScheduledAt(orderLike);
  if (!scheduledAt || !isPendingAcceptanceStatus(orderLike?.status)) return false;

  const scheduledTime = new Date(scheduledAt).getTime();
  if (!Number.isFinite(scheduledTime)) return false;

  return scheduledTime <= now + leadMinutes * 60000;
};
...
const isScheduledOrderReadyToPrepare = (
  orderLike = {},
  now = Date.now(),
) => {
  const scheduledAt = resolveScheduledAt(orderLike);
  const normalizedStatus = String(orderLike?.status || "").toLowerCase();
  if (!scheduledAt || normalizedStatus !== "scheduled") return false;

  const scheduledTime = new Date(scheduledAt).getTime();
  if (!Number.isFinite(scheduledTime)) return false;

  const leadMinutes = getScheduledPreparationLeadMinutes(orderLike);
  return scheduledTime - leadMinutes * 60000 <= now;
};
...
const hasReassignSignal = (orderLike = {}) => {
  const dispatchStatus = String(orderLike?.dispatch?.status || "").toLowerCase();
  if (dispatchStatus !== "unassigned") return false;
  const offeredTo = Array.isArray(orderLike?.dispatch?.offeredTo) ? orderLike.dispatch.offeredTo : [];
  return offeredTo.some((entry) => {
    const action = String(entry?.action || "").toLowerCase();
    return action === "rejected" || action === "timeout";
  });
};
...
const isTakeawayOrderLike = (order = {}) =>
  String(order?.fulfillmentType || "").toLowerCase() === "takeaway" ||
  String(order?.deliveryFleet || "").toLowerCase() === "takeaway" ||
  String(order?.type || "").toLowerCase() === "takeaway";

const getOrderFulfillmentLabel = (order = {}) => {
  if (isTakeawayOrderLike(order)) return "Takeaway";

  const deliveryFleet = String(order?.deliveryFleet || "").toLowerCase();
  if (deliveryFleet === "standard") return "Home Delivery";
  if (deliveryFleet === "express") return "Express Delivery";
...
const transformOrderForList = (order) => ({
  ...
  type: getOrderFulfillmentLabel(order),
  tableOrToken: null,
  ...
  assignmentNeedsReassign: hasReassignSignal(order),
  ...
  deliveryFleet: order.deliveryFleet || null,
});
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/45 px-4">
      <div className="w-full max-w-md rounded-2xl bg-white p-4 shadow-xl border border-slate-200">
        <h3 className="text-lg font-semibold text-slate-900">Assign Delivery Rider</h3>
        <p className="mt-1 text-sm text-slate-600">Select a rider to assign this order.</p>
        <div className="mt-4 space-y-2 max-h-64 overflow-y-auto">
          {riders.map((rider) => {
            const riderId = rider.partnerId || rider.id;
            const checked = String(selectedRiderId) === String(riderId);
            return (
              <label
                key={String(riderId)}
              ...
              >
                <input
                  ...
                  checked={checked}
                  onChange={() => onSelect?.(String(riderId))}
                />
                <div className="min-w-0">
                  <p className="text-sm font-medium text-slate-900 truncate">{rider.name || "Rider"}</p>
                  <p className="text-xs text-slate-600">{rider.phone || "No phone"}</p>
                </div>
              </label>
            );
          })}
...
function useAvailableExclusiveRidersByOrder(orders = [], refreshToken = 0) {
  const [availableExclusiveRidersByOrder, setAvailableExclusiveRidersByOrder] = useState({});
  ...
  useEffect(() => {
    ...
    const fetchAvailability = async () => {
      const unassignedOrders = orders.filter((order) => {
        ...
        if (!orderKey || dispatchStatus !== "unassigned") return false;
        if (isTakeawayOrderLike(order)) return false;
        return true;
      });

      if (!unassignedOrders.length) {
        if (isMounted) {
          setAvailableExclusiveRidersByOrder({});
        }
        return;
      }
      ...
      try {
        ...
        if (isMounted) {
          setAvailableExclusiveRidersByOrder(Object.fromEntries(results));
        }
      }
      catch {
        if (isMounted) setAvailableExclusiveRidersByOrder({});
      }
    };
  ...
  return availableExclusiveRidersByOrder;
}
  ...
  return (
    <div className="pt-4 pb-6">
      ...
      <AssignRiderModal
        ...
        riders={assignDialog.riders}
        selectedRiderId={selectedRiderId}
        onSelect={setSelectedRiderId}
        onClose={() => {
          if (assigningOrderIds[assignDialog.orderKey]) return;
          setAssignDialog({ open: false, orderKey: "", riders: [] });
          setSelectedRiderId("");
        }}
        onConfirm={handleConfirmAssign}
        loading={Boolean(assigningOrderIds[assignDialog.orderKey])}
      />
    </div>
  );
}
...
function AllOrders({ onSelectOrder, onCancel, refreshToken = 0 }) {
  ...
  const [startingPreparingOrderIds, setStartingPreparingOrderIds] = useState({});
  const [assigningOrderIds, setAssigningOrderIds] = useState({});
  const [autoAssigningOrderIds, setAutoAssigningOrderIds] = useState({});
  const [assignDialog, setAssignDialog] = useState({ open: false, orderKey: "", riders: [] });
  const [selectedRiderId, setSelectedRiderId] = useState("");
  const availableExclusiveRidersByOrder = useAvailableExclusiveRidersByOrder(orders, refreshToken);
  ...
  const handleStartPreparing = async ({ orderId, mongoId }) => {
    const orderKey = mongoId || orderId;
    if (!orderKey || startingPreparingOrderIds[orderKey]) return;
    ...
    try {
      ...
      debugError("Error starting preparation from All orders:", error);
      toast.error(
        error.response?.data?.message || "Failed to start preparing order",
      );
    } finally {
      setStartingPreparingOrderIds((prev) => ({ ...prev, [orderKey]: false }));
    }
  };

  const handleMarkDelivered = async ({ orderId, mongoId }) => {
    const orderKey = mongoId || orderId;
    if (!orderKey || markingDeliveredOrderIds[orderKey]) return;
    ...
    try {
      ...
      setMarkingDeliveredOrderIds((prev) => ({ ...prev, [orderKey]: false }));
    }
  };

  const handleAutoAssign = async ({ orderId, mongoId }) => {
    const orderKey = mongoId || orderId;
    ...
    try {
      ...
      setAutoAssigningOrderIds((prev) => ({ ...prev, [orderKey]: false }));
    }
  };

  const handleManualAssign = async ({ orderId, mongoId }) => {
    const orderKey = mongoId || orderId;
    ...
    try {
      ...
      const firstRiderId = riders[0]?.partnerId || riders[0]?.id || "";
      setSelectedRiderId(String(firstRiderId || ""));
      setAssignDialog({ open: true, orderKey, riders });
    } catch (error) {
  ...
  const handleConfirmAssign = async () => {
    const orderKey = String(assignDialog?.orderKey || "");
    if (!orderKey || !selectedRiderId) return;
    const chosen = (assignDialog?.riders || []).find(
      (r) => String(r?.partnerId || r?.id || "") === String(selectedRiderId),
    );
    if (!chosen) {
    ...
    try {
      ...
      setAssigningOrderIds((prev) => ({ ...prev, [orderKey]: false }));
    }
  };
          ...
          {orders.map((order) => {
            ...
            return (
              <OrderCard
                ...
                onAssignDelivery={handleManualAssign}
                hasExclusiveRiders={Boolean(
                  availableExclusiveRidersByOrder[String(order.mongoId || order.orderId || "")]
                )}
                isAutoAssigning={Boolean(autoAssigningOrderIds[order.mongoId || order.orderId])}
                isAssigning={Boolean(assigningOrderIds[order.mongoId || order.orderId])}
                isMarkingReady={Boolean(
                  markingReadyOrderIds[order.mongoId || order.orderId],
                )}
                isMarkingDelivered={Boolean(
                  markingDeliveredOrderIds[order.mongoId || order.orderId],
                )}
                isStartingPreparing={Boolean(
                  startingPreparingOrderIds[order.mongoId || order.orderId],
                )}
              />
            );
          })}
...
export default function OrdersMain() {
  ...
  const normalizePopupOrderForModal = (orderLike = {}, previous = null) => {
    ...
    const resolvedScheduledAt = resolveScheduledAt(source) || resolveScheduledAt(previous || {});
    ...
    return {
      ...
      type: source?.type || previous?.type || getOrderFulfillmentLabel(source),
      fulfillmentType:
        source?.fulfillmentType || previous?.fulfillmentType || null,
      deliveryFleet: source?.deliveryFleet || previous?.deliveryFleet || null,
    };
  };
  ...
  useEffect(() => {
    if (newOrder) {
      ...
      const status = String(newOrder.status || newOrder.orderStatus || "").toLowerCase();
      if (newOrder.isPreparationReminder || (status === "preparing" && newOrder.isScheduled)) {
        toast.info(
          `Time to start preparing Scheduled Order #${newOrder.orderId}!`,
          {
            duration: 10000,
            action: {
              label: "View",
              onClick: () => setActiveFilter(newOrder.isPreparationReminder ? "scheduled" : "preparing"),
            },
          }
        );
  ...
  const popupModalOrder = popupOrder || newOrder;
  const popupScheduledAt = resolveScheduledAt(popupModalOrder || {});
  const popupIsScheduled = Boolean(
          ...
          normalizeSelectedOrderForSheet(
            {
              ...
              customerName:
                ...
                "Guest",
              type: prev?.type || getOrderFulfillmentLabel(apiOrder),
  ...
  useEffect(() => {
    const handleRestaurantOrderStatusUpdated = (event) => {
      ...
      const dispatchStatusNow = String(
        payload?.dispatchStatus ?? payload?.dispatch_status ?? "",
      ).toLowerCase();
      const reasonType = String(payload?.reasonType || "").toLowerCase();
      const rawMessage = String(payload?.message || payload?.body || "").toLowerCase();
      const isPassOrTimeout =
        reasonType === "passed" ||
        reasonType === "timed_out" ||
        rawMessage.includes("passed") ||
        rawMessage.includes("timed out");
      if (dispatchStatusNow === "unassigned" && isPassOrTimeout) {
        toast.warning("Delivery partner passed/timed out. Please assign again.");
      }
      ...
      try {
        ...
        if (response.data?.success && response.data.data?.orders) {
          ...
          const targetOrders = response.data.data.orders.filter((order) => {
            ...
            const isPendingReview = isPendingAcceptanceStatus(order.status);
          ...
          if (
            ...
            const orderForPopup = {
              ...
              isScheduled: Boolean(orderToPopup.isScheduled || orderToPopup.scheduledAt),
              scheduledAt: orderToPopup.scheduledAt,
              fulfillmentType: orderToPopup.fulfillmentType || null,
              deliveryFleet: orderToPopup.deliveryFleet || null,
  ...
  const handleSelectOrder = (order) => {
    if (
      Boolean(order?.isScheduled || resolveScheduledAt(order)) &&
      isScheduledOrderReadyForAcceptance(order)
    ) {
      openOrderInAcceptanceModal(order);
      return;
    }
                    ...
                    <div className="mb-4 bg-green-50 border border-green-200 rounded-lg p-3 flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm font-semibold text-green-900 mt-0.5">
                          For{" "}
                          {new Date(popupScheduledAt).toLocaleString("en-US", {
                            day: "numeric",
                            month: "short",
                            hour: "2-digit",
                            minute: "2-digit",
                    ...
                    {popupIsScheduled && popupScheduledAt ? (
                      <>
                        <p className="text-xs font-semibold text-green-700 mt-1">
                          Scheduled for{" "}
                          {new Date(popupScheduledAt).toLocaleString("en-GB", {
                            day: "numeric",
                            month: "short",
                            year: "numeric",
                            hour: "2-digit",
...
function OrderCard({
  ...
  onAssignDelivery,
  onAutoAssign,
  hasExclusiveRiders = false,
  isAssigning = false,
  isAutoAssigning = false,
  isMarkingReady = false,
  isMarkingDelivered = false,
  isStartingPreparing = false,
}) {
  const normalizedStatus = String(status || "").toLowerCase();
  const isTakeaway = isTakeawayOrderLike({ fulfillmentType, deliveryFleet, type });
  const scheduledLabel = scheduledAt
    ? formatScheduledDateTime(scheduledAt)
    : "";
  const isScheduledPending =
    Boolean(scheduledAt) &&
    ["created", "confirmed", "scheduled"].includes(normalizedStatus);
  ...
  const assignmentBadgeText = isDeliveryAccepted
    ...
    : isDeliveryAssignedPending
      ? "Assigned"
      : assignmentNeedsReassign
      ? "Reassign Needed"
      : "Not Assigned";
  const isReady = normalizedStatus === "ready";
  const isPreparing = normalizedStatus === "preparing";
  const canMarkTakeawayDelivered = isTakeaway && isReady;
  const canStartPreparingScheduled =
    Boolean(isScheduled || scheduledAt) &&
        ...
        <div className="flex-1 flex flex-col justify-between min-h-[80px]">
          ...
          <div className="mt-2 flex items-end justify-between gap-2">
            <div className="flex flex-col gap-1">
              ...
              {!isTakeaway &&
                (isPreparing || isReady || normalizedStatus === "confirmed" || normalizedStatus === "scheduled") && (
                <div className="flex items-center gap-1.5 flex-wrap">
                  ...
                  {canShowManualAssign && onAssignDelivery && onAutoAssign && (
                    <>
                      <button
                        type="button"
                        onClick={(e) => {
                          e.stopPropagation();
                          onAssignDelivery({ orderId, mongoId, customerName });
                        }}
                        disabled={isAssigning || isAutoAssigning}
                        className="px-2 py-0.5 rounded-md text-[10px] font-semibold border border-blue-300 text-blue-700 bg-blue-50 disabled:opacity-60">
                      ...
                      {canShowAutoAssign && (
                        <button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            onAutoAssign({ orderId, mongoId, customerName });
                          }}
                          disabled={isAssigning || isAutoAssigning}
                          className="px-2 py-0.5 rounded-md text-[10px] font-semibold border border-purple-300 text-purple-700 bg-purple-50 disabled:opacity-60">
                          {isAutoAssigning ? "Sending..." : "Auto Assign"}
                        </button>
                      )}
                    </>
                  )}
            ...
            <div className="flex items-center gap-2">
              {canStartPreparingScheduled && onStartPreparing && (
                <button
                  type="button"
                  onClick={(e) => {
                    e.stopPropagation();
                    onStartPreparing({ orderId, mongoId, customerName });
                  }}
                  disabled={isStartingPreparing}
                  className="px-2.5 py-1 rounded-lg text-[11px] font-semibold border border-amber-600 text-amber-700 bg-amber-50 hover:bg-amber-100 disabled:opacity-60 disabled:cursor-not-allowed transition-colors">
                  {isStartingPreparing ? "Starting..." : "Start Preparing"}
                </button>
              )}
              {canMarkTakeawayDelivered && onMarkDelivered && (
                <button
                  type="button"
                  onClick={(e) => {
                    e.stopPropagation();
                    onMarkDelivered({ orderId, mongoId, customerName });
                  }}
                  disabled={isMarkingDelivered}
                  className="px-3 py-1.5 rounded-lg text-[11px] font-semibold border border-emerald-300 text-emerald-700 bg-emerald-50 disabled:opacity-60"
                >
...
function ScheduledOrders({ onSelectOrder, refreshToken = 0 }) {
      ...
      try {
        ...
        if (response.data?.success && response.data.data?.orders) {
          const scheduledOrders = response.data.data.orders
            .filter((order) => {
              const scheduledAt = resolveScheduledAt(order);
              if (!scheduledAt) return false;
              const status = String(order.status || "").toLowerCase();
              return ["created", "confirmed", "scheduled"].includes(status);
            })
            .map((order) => {
              const scheduledAt = resolveScheduledAt(order);
              return {
                ...
                photoAlt: order.items?.[0]?.name || "Order",
                paymentMethod: order.paymentMethod || order.payment?.method || null,
                isScheduled: Boolean(order.isScheduled || scheduledAt),
                scheduledAt,
                fulfillmentType: order.fulfillmentType || null,
                deliveryFleet: order.deliveryFleet || null,
              };
  ...
  return (
    <div className="pt-4 pb-6">
      <div className="flex items-baseline justify-between mb-3">
        <h2 className="text-base font-semibold text-black">Scheduled orders</h2>
        <span className="text-xs text-gray-500">{orders.length} pending</span>
      </div>
      {orders.length === 0 ? (
        <div className="text-center py-8 text-gray-500 text-sm">
          No scheduled orders
...
function TakeawayOrders({ onSelectOrder, onCancel, refreshToken = 0 }) {
  const [orders, setOrders] = useState([]);
  const [loading, setLoading] = useState(true);
  const [currentTime, setCurrentTime] = useState(new Date());
  const [markingReadyOrderIds, setMarkingReadyOrderIds] = useState({});
  ...
  useEffect(() => {
    ...
    const fetchOrders = async () => {
      try {
        ...
        if (response.data?.success && response.data.data?.orders) {
          const takeawayOrders = response.data.data.orders
            .filter((order) => {
              if (!isTakeawayOrderLike(order)) return false;
              const status = String(order.status || "").toLowerCase();
...
function PreparingOrders({
  ...
  const refreshOrders = async () => {
    ...
    const transformedOrders = preparingOrders.map((order) => {
      const initialETA = getOrderEtaMinutes(order);
      const preparingTimestamp = order.tracking?.preparing?.timestamp
        ? new Date(order.tracking.preparing.timestamp)
        : getOrderEtaStartTime(order);
      return {
          ...
          {orders.map((order) => {
            ...
            return (
              <OrderCard
                ...
                assignmentNeedsReassign={order.assignmentNeedsReassign}
