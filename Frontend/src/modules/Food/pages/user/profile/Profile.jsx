...
export default function Profile() {
  ...
  // Popup states synced with URL parameters to support back button closing
  const [searchParams] = useSearchParams();
  const dialogParam = searchParams.get("dialog");

  const vegModeOpen = dialogParam === "veg-mode";
  const appearanceOpen = dialogParam === "appearance";

  const setVegModeOpen = (open) => {
    if (open) {
      navigate("?dialog=veg-mode");
    } else {
      if (searchParams.get("dialog") === "veg-mode") {
        navigate(-1);
      }
    }
  };
  ...
  const setAppearanceOpen = (open) => {
    if (open) {
      navigate("?dialog=appearance");
    } else {
      if (searchParams.get("dialog") === "appearance") {
        navigate(-1);
      }
    }
  };
    ...
    <AnimatedPage className={`min-h-screen ${BRAND_THEME.tokens.profile.pageBackground}`}>
      <div className="max-w-md md:max-w-lg mx-auto px-4 sm:px-5 py-4 sm:py-5 pb-20 sm:pb-24">
        ...
        <div className="space-y-2 mb-3 mt-3">
          <Link to="/food/profile/subscriptions" className="block">
            <motion.div
              whileHover={{ x: 2 }}
                ...
                <CardContent className="p-4 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <motion.div
                      ...
                      <Gem className="h-5 w-5 text-amber-500 dark:text-amber-400" />
                    </motion.div>
          ...
          <Link to="/food/profile/my-subscriptions" className="block">
            <motion.div
              whileHover={{ x: 2 }}
              transition={{ duration: 0.2, type: "spring", stiffness: 300 }}>
              <Card className={optionCardClass}>
                ...
                <CardContent className="p-4 flex items-center justify-between">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className={rowLabelClass}>Food App Money</p>
        ...
        <DialogContent className="max-w-sm md:max-w-md lg:max-w-lg w-[calc(100%-2rem)] rounded-2xl p-0 overflow-hidden">
          <DialogHeader className="p-5 pb-3">
            ...
            <DialogDescription className="text-sm text-gray-500">
              Filter shops and dishes based on your dietary preferences
            </DialogDescription>
          </DialogHeader>
