    ...
    const fetchOrders = async ({ silent = false } = {}) => {
      try {
        ...
        if (ordersData.length > 0) {
          ...
          // For online payments, show only successfully paid orders in the user order list.
          // Keep COD/wallet orders unaffected.
          const visibleOrders = transformedOrders.filter((order) => {
            const paymentMethod = String(order?.paymentMethod || order?.payment?.method || '').toLowerCase()
            const paymentStatus = String(order?.payment?.status || '').toLowerCase()
            const isOnlineMethod = paymentMethod === 'razorpay' || paymentMethod === 'online' || paymentMethod === 'card'
            const isPaidOnline =
              paymentStatus === 'paid' ||
