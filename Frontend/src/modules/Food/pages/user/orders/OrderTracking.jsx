...
const getScheduledOrderLabel = (scheduledAtRaw) => {
  if (!scheduledAtRaw) return "Scheduled for later"
  const scheduledAt = new Date(scheduledAtRaw)
  if (Number.isNaN(scheduledAt.getTime())) return "Scheduled for later"

  return `Scheduled for ${scheduledAt.toLocaleDateString('en-IN', { month: 'short', day: 'numeric' })} at ${scheduledAt.toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })}`
}

const resolveScheduledAtFromOrder = (apiOrder, previousOrder = null) => {
  const candidates = [
    apiOrder?.scheduledAt,
    apiOrder?.scheduleAt,
    apiOrder?.scheduledFor,
    apiOrder?.deliverySchedule?.scheduledAt,
    previousOrder?.scheduledAt,
  ]
...
const transformOrderForTracking = (apiOrder, previousOrder = null, explicitRestaurantCoords = null, explicitRestaurantAddress = null) => {
  ...
  return {
    ...
    fulfillmentType: apiOrder?.fulfillmentType || previousOrder?.fulfillmentType || 'delivery',
    ...
    estimatedDeliveryTime:
      ...
      null,
    isScheduled:
      typeof apiOrder?.isScheduled === "boolean"
        ? apiOrder.isScheduled
        : Boolean(previousOrder?.isScheduled || scheduledAt),
    isActivated:
      typeof apiOrder?.isActivated === "boolean"
        ? apiOrder.isActivated
        : Boolean(previousOrder?.isActivated),
    scheduledAt,
...
export default function OrderTracking() {
  ...
  const scheduledAtMs = order?.scheduledAt
    ? new Date(order.scheduledAt).getTime()
    : Number.NaN
  const hasFutureScheduledTime =
    Number.isFinite(scheduledAtMs) && scheduledAtMs > Date.now()
  const shouldShowPlacedEta =
    typeof estimatedTime === 'number' && estimatedTime > 0
  const placedSubtitle = hasFutureScheduledTime
  ...
  const statusConfig = {
    scheduled: {
      title: order?.fulfillmentType === 'takeaway' ? "Pickup Scheduled" : "Delivery Scheduled",
      subtitle: getScheduledOrderLabel(order?.scheduledAt),
      color: BRAND_THEME.colors.brand.primary,
      iconType: 'food'
    },
    placed: {
      title: "Order Placed",
      subtitle: placedSubtitle,
      color: BRAND_THEME.colors.brand.primary,
      iconType: 'food'
    },
    confirmed: {
    ...
    delivered: {
      title: order?.fulfillmentType === 'takeaway' ? "Order Picked Up" : "Order delivered",
            ...
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-sm font-semibold text-gray-900">
                  {order?.fulfillmentType === 'takeaway' ? 'Pickup OTP' : 'Delivery OTP'}
                </p>
      ...
      <div className="max-w-4xl mx-auto px-4 md:px-6 lg:px-8 py-4 md:py-6 space-y-4 md:space-y-6 pb-24 md:pb-32">
        ...
        >
          <p className="text-brand-800 font-medium">
            {order?.fulfillmentType === 'takeaway' ? 'All your pickup details in one place' : 'All your delivery details in one place'}
          </p>
        </motion.div>
        ...
        >
          ...
          {order?.fulfillmentType !== 'takeaway' && canManageDeliveryInstructions && !hasDeliveryInstructions && (
            <SectionItem
              icon={MessageSquare}
          ...
          <div className="p-6 pt-4 space-y-6 max-h-[70vh] overflow-y-auto">
            ...
            {order?.note && (
              <div className="bg-brand-50/50 rounded-xl p-4 border border-brand-100 flex gap-3">
                ...
                <div>
                  <p className="text-xs text-brand-600 font-bold uppercase tracking-wider mb-1">
                    {order?.fulfillmentType === 'takeaway' ? 'Pickup Notes' : 'Delivery Instructions'}
                  </p>
