...
export default function AddMoneyModal({ open, onOpenChange, onSuccess, initialAmount = "" }) {
  const [amount, setAmount] = useState(initialAmount ? initialAmount.toString() : "")
  const [loading, setLoading] = useState(false)
  const [processing, setProcessing] = useState(false)

  // Sync state if initialAmount changes when opening
  useEffect(() => {
    if (open) {
                  ...
                  return (
                    <Button
                      ...
                      className={`h-10 transition-all duration-200 ${
                        isSelected
                          ? "bg-green-600 hover:bg-green-700 border-green-600 text-white hover:text-white dark:bg-green-600 dark:hover:bg-green-700 dark:text-white dark:border-green-600"
