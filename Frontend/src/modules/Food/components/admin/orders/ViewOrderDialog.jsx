...
const resolveScheduledAt = (order = {}) =>
  order?.scheduledAt ||
...
const getOrderTypeDetails = (order = {}) => {
  ...
  const isScheduled = Boolean(order?.isScheduled || scheduledAt)
  ...
  const isTakeaway =
    fulfillmentTypeRaw === "takeaway" ||
    fulfillmentTypeRaw === "pickup"

  return {
    isScheduled,
    scheduledAt,
    scheduleLabel: isScheduled ? "Scheduled" : "Instant",
    modeLabel: isTakeaway ? "Takeaway" : "Delivery",
    orderTypeLabel: `${isScheduled ? "Scheduled" : "Instant"} + ${isTakeaway ? "Takeaway" : "Delivery"}`,
  }
}
...
export default function ViewOrderDialog({
  ...
  useEffect(() => {
    ...
    const fetchFullOrder = async () => {
      ...
      try {
        const lookupId = order?.mongoId || order?._id || order?.id || order?.orderId
        if (!lookupId) return
        const response = await adminAPI.getOrderById(lookupId)
        const fetched =
          response?.data?.data?.order ||
          response?.data?.order ||
          response?.data?.data ||
          null
        if (mounted && fetched && typeof fetched === "object") {
          setFullOrder((prev) => ({ ...(prev || {}), ...fetched }))
        }
      } catch (err) {
        debugWarn("Failed to fetch full order details for modal", err?.message || err)
      }
    }
    fetchFullOrder()
    return () => {
  ...
  const pickupProofImageUrl = useMemo(
    () =>
      ...
      modalOrder?.billImage ||
      modalOrder?.deliveryState?.billImageUrl ||
      modalOrder?.deliveryState?.pickupProofImageUrl ||
      modalOrder?.pickupProofImageUrl ||
      modalOrder?.pickupPhotoUrl ||
      modalOrder?.statusHistory?.find?.((entry) => entry?.meta?.billImageUrl)?.meta?.billImageUrl ||
      "",
    [modalOrder],
  )
  ...
  if (pickupProofImageUrl) {
    debugLog('?? Bill Image URL found:', pickupProofImageUrl)
  } else {
    debugLog('?? Bill Image URL not found in order:', {
      orderId: modalOrder.orderId,
      hasBillImageUrl: !!pickupProofImageUrl,
      orderKeys: Object.keys(modalOrder)
    })
  }
  ...
  const recipientSourceAddress = modalOrder.address || modalOrder.deliveryAddress || modalOrder.customerAddress || {}
        ...
        <div className="px-6 py-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
              ...
              <div className="space-y-1">
                ...
                <p className="text-sm font-semibold text-slate-900">{orderTypeDetails.orderTypeLabel}</p>
              ...
              {orderTypeDetails.isScheduled && orderTypeDetails.scheduledAt && (
                <div className="space-y-1">
                  ...
                  <p className="text-sm font-medium text-slate-900">
                    {new Date(orderTypeDetails.scheduledAt).toLocaleString("en-GB", {
                      day: "2-digit",
                      month: "short",
                      year: "numeric",
                      hour: "2-digit",
              ...
              {orderTypeDetails.modeLabel === "Takeaway" && (
                <div className="space-y-1">
                  <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider flex items-center gap-2">
                    <Truck className="w-4 h-4" />
                    Fulfillment
                  </p>
                  <p className="text-sm font-medium text-slate-900">Customer Pickup / Takeaway</p>
                </div>
              )}
              {orderTypeDetails.modeLabel === "Delivery" && (
                <div className="space-y-1">
                  <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider flex items-center gap-2">
                    <Truck className="w-4 h-4" />
                    Fulfillment
          ...
          <div className="border-t border-slate-200 pt-4">
            <h3 className="text-sm font-semibold text-slate-700 mb-4 flex items-center gap-2">
              ...
              Pickup Photo (Captured by Delivery Boy)
            </h3>
            {pickupProofImageUrl ? (
              <div className="space-y-3">
                <div className="relative w-full max-w-md border-2 border-slate-300 rounded-xl overflow-hidden bg-white shadow-sm">
                  ...
                  <div className="error-message hidden p-6 text-center text-slate-500 text-sm bg-slate-50">
                    ...
                    Failed to load pickup photo
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <a
                    href={pickupProofImageUrl}
                    target="_blank"
              ...
              <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-800">
                Pickup photo not uploaded for this order.
              </div>
            )}
          </div>
