...
import { Eye, ArrowUpDown, Phone, User } from "lucide-react"
          ...
          <tbody className="bg-white divide-y divide-slate-100">
            {paginatedOrders.map((order, index) => (
              ...
              >
                ...
                {visibleColumns.shopName && (
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className="text-sm font-medium text-slate-700">{order.shopName}</span>
                  </td>
                )}
                {visibleColumns.deliveryBoy && (
                  <td className="px-6 py-4">
                    ...
                    ) : order.requestedDeliveryBoyName && order.dispatchStatus === "assigned" && order.latestOfferAction === "offered" ? (
                      <div className="flex flex-col">
                        <div className="flex items-center gap-1.5 mb-1">
                          <User className="w-3.5 h-3.5 text-amber-600" />
                          <span className="text-sm font-medium text-amber-700">{order.requestedDeliveryBoyName}</span>
                        </div>
                        <div className="flex items-center gap-1.5 mb-1">
                          <Phone className="w-3.5 h-3.5 text-amber-600" />
                          <span className="text-xs text-amber-700 font-medium">{order.requestedDeliveryBoyNumber || "N/A"}</span>
                        </div>
                ...
                {visibleColumns.actions && (
                  <td className="px-6 py-4 whitespace-nowrap text-center">
                    <div className="flex items-center justify-center gap-2">
                      {order.status === "User Unavailable Review" && (
                        <>
                          <button
                            onClick={() => onAdminStatusChange?.(order, "approve_user_unavailable")}
                            disabled={actionLoadingKey === `status:${order.orderMongoId}:approve_user_unavailable`}
                            className="px-2.5 py-1 rounded-md text-xs font-semibold bg-emerald-100 text-emerald-700 hover:bg-emerald-200 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                            title="Approve"
                          >
                            Approve
                          </button>
                          <button
                            onClick={() => onAdminStatusChange?.(order, "reject_user_unavailable")}
                            disabled={actionLoadingKey === `status:${order.orderMongoId}:reject_user_unavailable`}
                            className="px-2.5 py-1 rounded-md text-xs font-semibold bg-rose-100 text-rose-700 hover:bg-rose-200 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                            title="Reject"
                          >
                            Reject
                          </button>
