...
export async function generateOrderInvoicePDF(order) {
  try {
    ...
    const discountAmount = toNumber(
      ...
      order.pricing?.autoOfferDiscount
    )
    const couponCode = String(
      ...
      order.pricing?.autoAppliedOffer?.code ||
      ""
    ).trim()
    ...
    doc.setDrawColor(226, 232, 240)
    doc.setFillColor(248, 250, 252)
    doc.roundedRect(summaryLeftMargin - 4, summaryStartY - 5, summaryCardWidth, summaryCardHeight, 2, 2, "FD")
    autoTable(doc, {
