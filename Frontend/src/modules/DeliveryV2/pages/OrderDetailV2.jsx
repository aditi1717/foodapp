...
const ATTEMPT_TIMER_SECONDS = 5 * 60; // 5 minutes
const ATTEMPT_TIMER_STORAGE_PREFIX = 'delivery_v2_attempt_timer';
...
const OrderDetailV2 = () => {
  ...
  const attemptTimerStorageKey = useMemo(
    () => `${ATTEMPT_TIMER_STORAGE_PREFIX}:${resolvedLookupOrderId || routeOrderId || 'unknown'}`,
    [resolvedLookupOrderId, routeOrderId],
  );

  const clearAttemptTimerStorage = useCallback(() => {
    try {
      localStorage.removeItem(attemptTimerStorageKey);
    } catch (error) {
      console.error('Failed to clear attempt timer storage', error);
    }
  }, [attemptTimerStorageKey]);

  const startAttemptCountdown = useCallback((endsAtMs) => {
  ...
  const startAttemptTimer = useCallback(() => {
    const endsAtMs = Date.now() + ATTEMPT_TIMER_SECONDS * 1000;
    try {
      localStorage.setItem(
        attemptTimerStorageKey,
        JSON.stringify({ endsAtMs, orderId: resolvedLookupOrderId || routeOrderId || '' }),
      );
    } catch (error) {
      console.error('Failed to persist attempt timer', error);
    }
    startAttemptCountdown(endsAtMs);
  }, [attemptTimerStorageKey, resolvedLookupOrderId, routeOrderId, startAttemptCountdown]);
  ...
  useEffect(() => {
    if (!attemptTimerStorageKey || attemptSubmitted) return;
    try {
      const raw = localStorage.getItem(attemptTimerStorageKey);
      if (!raw) return;
      const parsed = JSON.parse(raw);
      const endsAtMs = Number(parsed?.endsAtMs || 0);
      ...
      console.error('Failed to restore attempt timer', error);
      clearAttemptTimerStorage();
    }
  }, [attemptSubmitted, attemptTimerStorageKey, clearAttemptTimerStorage, startAttemptCountdown]);
  ...
  }, [clearActiveOrder, clearAttemptTimerStorage, proofPhoto, resolvedLookupOrderId, routeOrderId]);
            ...
            <div className="rounded-2xl border border-slate-200 bg-white overflow-hidden">
              ...
              {!attemptSubmitted && attemptPhase === 'normal' && (
                ...
                >
                  ...
                  <span className="text-[11px] font-bold text-orange-500 bg-orange-50 rounded-lg px-2 py-1">5 min</span>
                </button>
              )}
          ...
          <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 p-4 sm:items-center">
            <div className="w-full max-w-md rounded-2xl bg-white p-4 shadow-2xl">
              ...
              <p className="text-xs text-slate-500">
                Ask the customer for the OTP from their app and enter it here. The order will be marked delivered only after OTP verification.
              </p>
              <input
                type="text"
                inputMode="numeric"
