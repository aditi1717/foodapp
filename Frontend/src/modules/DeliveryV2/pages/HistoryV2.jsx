...
const getTripEarning = (trip) => {
  const isCancelled = ['cancelled', 'rejected'].includes(String(trip?.status || '').toLowerCase());
  const isCompensated = isUserUnavailableTrip(trip);
  const rawStatus = String(trip?.rawOrderStatus || trip?.orderStatus || trip?.status || '').toLowerCase();
  const isUnderReview = rawStatus.includes('review') || rawStatus.includes('unavailable');

  if (isCancelled && !isCompensated && !isUnderReview) return 0;
